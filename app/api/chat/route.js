...
export async function POST(req) {
  try {
    ...
    const jobContext = jobs.map((job, index) => {
      ...
      return `[JOB #${index + 1}] Role: ${job.title} at ${job.companyName || "Confidential"} | Pay: ₹${job.salary?.min}-₹${job.salary?.max} | Skills: ${skills}`;
    }).join("\n");
    ...
    if (!pythonResponse.ok) throw new Error("Python Server Failed");
    const aiData = await pythonResponse.json();

    // 6. Save AI's Response to MongoDB so it shows up in your sidebar history later
    chat.messages.push({ role: "model", content: aiData.reply });
    await chat.save();
