...
import { 
  ...
  MapPin, ChevronRight, CheckCircle, Clock 
} from "lucide-react";
...
export default function RecruiterDashboard() {
  ...
  const [isQueued, setIsQueued] = useState(false); // Tracks AWS SQS Background state

  useEffect(() => { 
    const fetchJobs = async () => {
      try {
        const res = await fetch(`/api/jobs/get?recruiterId=${session.user.id}&t=${Date.now()}`); 
        if (res.ok) {
  ...
  const handleGetSuggestions = async (job) => {
    ...
    setSuggestions([]);
    setIsQueued(false); // Reset queue state on new click
    setIsPolling(false);
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Check if Python pushed it to the AWS SQS queue
  ...
  // Add this state to the top of your component with your other useState hooks
  const [isPolling, setIsPolling] = useState(false);

  useEffect(() => {
    let interval;

    if (isPolling && selectedJob) {
      interval = setInterval(async () => {
        try {
          console.log("Checking database for AI results...");
          const res = await fetch(`/api/jobs/${selectedJob._id}/suggestions?t=${Date.now()}`);
          
          if (res.ok) {
            const data = await res.json();
            if (data.suggestions && data.suggestions.length > 0) {
              setSuggestions(data.suggestions); // Show the candidates!
              setIsQueued(false); // Turn off the loading screen
              setIsPolling(false); // Kill the timer
            }
          }
        } catch (error) {
    ...
    //at the end cleanup the timer fucntion
    return () => clearInterval(interval);
  }, [isPolling, selectedJob]);
                ...
                <div className="p-6 flex-1 overflow-y-auto bg-slate-50/50 dark:bg-black/20">
                  {loadingSuggestions ? (
                    <div className="flex flex-col items-center justify-center h-64 space-y-4">
                      ...
                      <p className="text-sm text-slate-500 animate-pulse">Sending payload to AWS Queue...</p>
                    </div>
                  ) : isQueued ? (
                    <motion.div initial={{opacity:0}} animate={{opacity:1}} className="flex flex-col items-center justify-center h-64 space-y-4 text-center">
                      <div className="w-16 h-16 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center mb-2">
                        <Clock className="text-blue-600 w-8 h-8 animate-pulse" />
                      </div>
                      <h3 className="text-lg font-bold text-slate-900 dark:text-white">AI is crunching the numbers!</h3>
                      <p className="text-sm text-slate-500 max-w-md">Your request has been queued in our background worker. Feel free to navigate away; the matches will be saved to this job posting shortly.</p>
                    </motion.div>
