...
import { 
  ...
  MoreHorizontal, Calendar, MapPin, 
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-black font-sans text-slate-900 dark:text-slate-100">
      ...
      <header className="bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 sticky top-0 z-30">
        <div className="mx-auto flex h-auto max-w-7xl flex-col gap-4 px-4 py-4 sm:px-6 lg:h-20 lg:flex-row lg:items-center lg:justify-between lg:px-8 lg:py-0">
          <div>
          ...
          <Link 
            href="/admin/post" 
            className="flex w-full items-center justify-center gap-2 rounded-full bg-blue-600 px-5 py-2.5 font-semibold text-white shadow-lg shadow-blue-600/25 transition-all active:scale-95 hover:bg-blue-500 sm:w-auto"
          >
            <Plus size={18} />
            Post New Job
          </Link>
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="relative w-full sm:w-auto">
              ...
              <input 
                ...
                className="w-full rounded-lg border border-slate-200 bg-white py-2 pl-10 pr-4 text-sm outline-none focus:ring-2 focus:ring-blue-500 dark:border-zinc-800 dark:bg-zinc-900 sm:w-64"
              />
            </div>
          </div>
          ...
          */}
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 shadow-sm">
            ...
            {/* Table Header */}
            <div className="hidden grid-cols-12 gap-4 rounded-t-2xl border-b border-slate-200 bg-slate-50/50 px-6 py-4 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:border-zinc-800 dark:bg-zinc-900/50 md:grid">
                ...
                <AnimatePresence>
                  {jobs.map((job) => (
                    <motion.li 
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="group relative px-4 py-4 transition-colors hover:bg-slate-50 dark:hover:bg-zinc-800/50 md:grid md:grid-cols-12 md:items-center md:gap-4 md:px-6"
                    >
                      {/* Job Info */}
                      ...
                      <div className="mt-4 flex items-center justify-between md:col-span-2 md:mt-0 md:block">
                        <span className="text-xs font-semibold uppercase tracking-wide text-slate-400 md:hidden">Applicants</span>
                      ...
                      {/* Status */}
                      <div className="mt-3 flex items-center justify-between md:col-span-2 md:mt-0 md:block md:text-center">
                      ...
                      {/* Date */}
                      <div className="mt-3 flex items-center justify-between text-sm text-slate-500 md:col-span-2 md:mt-0 md:block">
                        <span className="text-xs font-semibold uppercase tracking-wide text-slate-400 md:hidden">Posted</span>
                      ...
                      <div className="relative mt-4 flex items-center justify-end gap-2 md:col-span-1 md:mt-0 md:text-right">
                          <div className="flex w-full gap-2 md:hidden">
                            <button
                              onClick={() => router.push(`/admin/edit/${job._id}`)}
                              className="flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 dark:border-zinc-700 dark:text-slate-200 dark:hover:bg-zinc-800"
                            >
                              Edit
                            </button>
                            <button
                              onClick={() => handleDelete(job._id)}
                              className="flex-1 rounded-lg border border-red-200 px-3 py-2 text-sm font-medium text-red-600 transition-colors hover:bg-red-50 dark:border-red-900/50 dark:hover:bg-red-900/20"
                            >
                              Delete
                            </button>
                          </div>
                          <button 
                            ...
                            className="relative z-10 hidden cursor-pointer rounded-lg p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-zinc-800 dark:hover:text-white md:inline-flex"
                          >
